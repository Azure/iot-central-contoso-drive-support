...
const useCommand = (payload: any) => {
    ...
    const executeCommand = async () => {
        ...
        try {
            ...
            const res = await dataContext.sendCommand(payload.host, payload.id, payload.name, payload.body, token);
            setData(res);
            setTimeout(() => {
                setData(null);
            }, 5000);
    ...
    return [loading, data, error, executeCommand];
};
